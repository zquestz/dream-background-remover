...
"""
Build translation files for Dream Background Remover plugin
...
def compile_translations() -> bool:
    ...
    for po_file in sorted(po_files):
        ...
        mo_file = target_dir / 'dream-background-remover.mo'
    ...
    print(f"\nCompilation complete: {success_count}/{len(po_files)} "
          f"files compiled successfully")
...
def create_template_files() -> bool:
    ...
    if not pot_file.exists():
        print("No template found: locale/dream-background-remover.pot")
        print("Run 'python3 scripts/update-pot.py' first")
        return False

    print("To create a new language translation:")
    print("  cp locale/dream-background-remover.pot locale/[LANG].po")
